...
bool Bank::getCustomersAccountInfo(const std::string accountNumber, float &balance, float &credit, float &disposalBalance) const {
	return client->getAccountInfo(accountNumber,  balance, credit, disposalBalance);
}
...
void Bank::getCustomersSummary(std::map <std::string, std::vector<int>> &map) {
	return client->getSummary(map);
}

bool Bank::createNewCustomerAccount() {
	return client->createNewAccount();
}
