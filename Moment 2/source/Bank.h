...
public:
	...
	bool getCustomersAccountInfo(const std::string accountNumber, float &balance, float &credit, float &disposalBalance) const;
	...
	void getCustomersSummary(std::map <std::string, std::vector<int>> &map);
	bool createNewCustomerAccount();
