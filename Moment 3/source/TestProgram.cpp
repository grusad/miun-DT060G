...
void TestProgram::selectAccountToInfo() {
	...
	float balance = 0, credit = 0, disposable = 0, interest = 0;
	int maxWithdraw = -1;
	Type type;

	if (bank.getCustomersAccountInfo(Input::getString(), balance, credit, disposable, interest, maxWithdraw, type)) {
		printAccountInfo(type, balance, credit, disposable, interest, maxWithdraw);
	}
	else {
...
void TestProgram::createAccount() {
	...
	Menu menu;
	menu.add(MenuItem("Transaction Account", true));
	menu.add(MenuItem("Savings Account", true));
	menu.add(MenuItem("Longterm Savings Account", true));
	...
	menu.print();
	int type = menu.getAlternative();

	if (type == 1) accountType = TRANSACTION;
	else if (type == 2) accountType = SAVINGS;
	else accountType = LONG_SAVINGS;

	if (bank.createNewCustomerAccount(accountType)) {
...
void TestProgram::printAccountInfo(const Type type, const float balance, const float credit, const float disposable, float interest, int maxWidthdraw) const {
	std::string accountType;
	if (type == TRANSACTION) accountType = "Transaction account";
	if (type == SAVINGS) accountType = "Savings account";
	if (type == LONG_SAVINGS) accountType = "Longterm savings account";
	std::cout << "Account type: " + accountType << std::endl;
	std::cout << "Balance: " << balance << std::endl;
	std::cout << "Credit: " << credit << std::endl;
	std::cout << "Disposable: " << disposable << std::endl;
	std::cout << "Interest: " << interest << std::endl;
	std::cout << "Annual withdrawls: ";
	if (maxWidthdraw == -1) std::cout << "UNLIMITED." << std::endl;
	else std::cout << maxWidthdraw << std::endl;
}
...
void TestProgram::printSummary() {
	...
	std::vector<std::string> allAccounts = bank.getCustomersAccountNumbers();

	for (auto i : allAccounts) {

		float balance = 0, credit = 0, disposable = 0, interest = 0;
		int maxWithdraw = -1;
		Type type;

		if (bank.getCustomersAccountInfo(i, balance, credit, disposable, interest, maxWithdraw, type)) {
			printAccountInfo(type, balance, credit, disposable, interest, maxWithdraw);
		}
